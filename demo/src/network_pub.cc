#include <fastrtps/attributes/ParticipantAttributes.h>
#include <fastrtps/attributes/PublisherAttributes.h>
#include <fastdds/dds/domain/DomainParticipantFactory.hpp>
#include <fastdds/dds/publisher/Publisher.hpp>
#include <fastdds/dds/publisher/qos/PublisherQos.hpp>
#include <fastdds/dds/publisher/DataWriter.hpp>
#include <fastdds/dds/publisher/qos/DataWriterQos.hpp>

#include <fastdds/dds/publisher/DataWriterListener.hpp>
#include <fastdds/dds/topic/TypeSupport.hpp>
#include <fastdds/dds/domain/DomainParticipant.hpp>

#include <fastdds/rtps/transport/UDPv4TransportDescriptor.h>

// #include </rtps/transport/UDPv4Transport.h>
#include <UDPv4Transport.h>
#include <UDPSenderResource.hpp>
// /home/linux/Data/DDS/src/fastrtps/src/cpp/rtps/transport
#include <NetworkFactory.h>

#include <CDRMessage_t.h>

int main(int argc, char ** argv) {

    auto user_udp_transport_dsp = std::make_shared<eprosima::fastdds::rtps::UDPv4TransportDescriptor>();
    eprosima::fastdds::rtps::TransportInterface * transport_udp_interface =  user_udp_transport_dsp->create_transport();

    // eprosima::fastdds::rtps:
    eprosima::fastdds::rtps::UDPv4Transport * udp_transport = static_cast<eprosima::fastdds::rtps::UDPv4Transport*>(transport_udp_interface);
    bool is_init = udp_transport->init();


    //eprosima::fastdds::rtps::SendResourceList &sender_resource_list
    //std::vector<std::unique_ptr<fastrtps::rtps::SenderResource>>;
    eprosima::fastdds::rtps::SendResourceList send_res_list;

    // std::unique_ptr<eprosima::fastrtps::rtps::SenderResource> sr = std::make_unique<eprosima::fastdds::rtps::UDPSenderResource>(udp_transport,);
    // send_res_list.push_back(sr);

    // //eprosima::fastrtps::rtps::Locator_t;
    // eprosima::fastdds::rtps::Locator locator(1111);
    // locator.kind = LOCATOR_KIND_UDPv4;
    // locator.port = 1111;
    // // locator.set_address();

    // eprosima::fastdds::rtps::UDPSenderResource;

    eprosima::fastdds::rtps::LocatorList locator_list;
    // locator_list.push_back(locator);
    

    // auto lb = locator_list.begin();
    // eprosima::fastdds::rtps::Locators locators(lb);

    eprosima::fastrtps::rtps::RTPSParticipantAttributes attr;
    eprosima::fastrtps::rtps::NetworkFactory network_factory(attr);
    network_factory.generate_locators(36000,1,locator_list);

    auto desp = udp_transport->get_configuration();
    network_factory.RegisterTransport(desp);

    for(auto & loc : locator_list) {
        bool is_ok = network_factory.build_send_resources(send_res_list,loc);
    }

    // std::vector<eprosima::fastdds::rtps::LocatorsIterator *> dds_locators;
    // dds_locators.push_back(&locators);

    //CDR
    eprosima::fastrtps::rtps::CDRMessage_t cdr_msg;
    eprosima::fastrtps::rtps::CDRMessage::initCDRMsg(&cdr_msg);
    using namespace eprosima::fastrtps::rtps;
    bool is_add = CDRMessage::add_string(&cdr_msg,"Hello");

    // eprosima::fastrtps::string_255 read_msg;
    cdr_msg.pos = 0;
    std::string read_msg;
    bool is_read = CDRMessage::readString(&cdr_msg,&read_msg);
    
    


    eprosima::fastrtps::rtps::octet data[]={'h','e'};

    std::chrono::_V2::steady_clock::time_point tp;

    for(;;){
        for(auto &send:send_res_list){
            eprosima::fastdds::rtps::Locators begin(locator_list.begin());
            eprosima::fastdds::rtps::Locators end(locator_list.end());
            bool is_send = send->send(data,2,&begin,&end,tp);
            // bool is_send = send->send(data,2,&eprosima::fastdds::rtps::Locators(locator_list.begin()),&eprosima::fastdds::rtps::Locators(locator_list.end()),tp);
            std::cout<<"pub data:"<<data<<"is_send:"<< is_send<<std::endl;
            
        }
        std::this_thread::sleep_for(std::chrono::seconds(1));
    }

   

    return 0;
}